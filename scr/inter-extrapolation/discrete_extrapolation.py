...
y_pred = interpolation_df['GNNGH_T'].to_numpy()
...
extrapolation_df['error_gnngh'] = np.abs(extrapolation_df['log-gamma'].to_numpy() - extrapolation_df['GNNGH_T'].to_numpy())
extrapolation_df = extrapolation_df.sort_values(by=['error_gnngh'])

n_best_systems = 73
y_true = extrapolation_df['log-gamma'].to_numpy()
y_pred = extrapolation_df['GNNGH_T'].to_numpy()

y_true_good = extrapolation_df['log-gamma'].to_numpy()[:n_best_systems]
y_pred_good = extrapolation_df['GNNGH_T'].to_numpy()[:n_best_systems]
...
y_pred_bad = extrapolation_df['GNNGH_T'].to_numpy()[-(extrapolation_df.shape[0]-n_best_systems):]
...
df = pd.read_csv(folder +'/edge_Brouwer_GNNGH_for_parity_Nsystems.csv')
fig = plt.figure(figsize=(7, 6))
...
plt.close(fig)
fig.savefig(folder +'/parity_extrapolation_Brouwer_GNNGH.png', dpi=300, format='png')
# fig.savefig(folder +'/parity_extrapolation_Brouwer_GNNGH.svg', dpi=300, format='svg')




bins_classes = np.arange(0, 225, 25)
bins_mae =np.zeros(bins_classes.shape[0])
for i in range(bins_mae.shape[0]):
    df_feasible = df[df['Same_key_class_n'] >= bins_classes[i]]
    bins_mae[i] = MAE(df_feasible['log-gamma'].to_numpy(), df_feasible['GNNGH_T'].to_numpy())
...
plt.tight_layout()
plt.close(fig)
fig.savefig(folder +'/Threshold_n_systems_extrapolation_Brouwer_GNNGH.png', dpi=650, format='png')
# fig.savefig(folder +'/Threshold_n_systems_extrapolation_Brouwer_GNNGH.svg', dpi=650, format='svg')
...
fig.colorbar(im, cax=cax, orientation='horizontal')
plt.close(fig)
fig.savefig(folder +'/parity_extrapolation_Brouwer_GNNGH_Tanimoto.png', dpi=300, format='png')
# fig.savefig(folder +'/parity_extrapolation_Brouwer_GNNGH_Tanimoto.svg', dpi=300, format='svg')
...
plt.tight_layout()
plt.close(fig)
fig.savefig(folder + '/Threshold_tanimoto_extrapolation_Brouwer_GNNGH.png', dpi=650, format='png')
# fig.savefig(folder + '/Threshold_tanimoto_extrapolation_Brouwer_GNNGH.svg', dpi=650, format='svg')
